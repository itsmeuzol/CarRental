...
exports.generateMonthlyReport = async (req, res) => {
  try {
    ...
    doc
      .fontSize(20)
      .text(
        `Monthly Business Report - ${moment(`${year}-${month}`).format(
          "MMMM YYYY"
        )}`,
        { align: "center" }
      );
...
exports.generateReport = async (req, res) => {
  try {
    ...
    if (reportScope === "monthly" && !month) {
      return res
        .status(400)
        .json({ message: "Month is required for monthly reports" });
    }
    ...
    doc
      .fontSize(20)
      .text(`Business Report - ${titleDate}`, { align: "center" });
    doc.moveDown();
...
async function fetchCustomerFeedbackData(month, year, scope) {
  ...
  return result.length > 0
    ? result[0]
    : { totalReviews: 0, averageRating: 0, feedbackDetails: [] };
}
...
function renderReportSections(doc, data) {
  ...
  doc
    .fontSize(12)
    .text(
      `Total Listings/Bookings: ${
        data.totalListings || data.totalBookings || "N/A"
      }`
    )
    .text(
      `Total Revenue: $${
        data.totalSalesValue || data.totalRentalRevenue || "N/A"
      }`
    );
}
...
function renderReportSections(doc, data) {
  ...
  if (data.totalReviews && data.totalReviews > 0) {
    ...
    data.feedbackDetails.forEach((feedback, index) => {
      doc.text(
        `${index + 1}. User: ${feedback.user}, Rating: ${
          feedback.rating
...
function addReportSummary(doc, data) {
  doc.moveDown().fontSize(14).text("Summary", { underline: true }).fontSize(12);
